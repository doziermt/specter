...
`nmap`, `masscan`, and `eyewitness` CLI commands via the Python [subprocess](https://docs.python.org/3/library/subprocess.html) library.
It also contains all intermediary and 3rd-party application output files. "Intermediary" files are those generated by Specter for feeding
"clean target list files" from one application to the subsequent one. The "3rd-party application" files include those generated by `nmap`,
`masscan` and `eyewitness`.
